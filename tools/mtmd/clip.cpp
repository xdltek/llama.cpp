...
struct clip_init_result clip_init(const char * fname, struct clip_context_params ctx_params) {
    ...
    try {
        ...
        if (loader.has_vision) {
            ...
            ggml_backend_dev_t dev = ggml_backend_get_device(ctx_vision->backend);
            ggml_backend_reg_t reg = dev ? ggml_backend_dev_backend_reg(dev) : nullptr;
            if (reg) {
                using ggml_backend_rpp_set_params_t = void (*)(ggml_backend_t backend, const int domain, const int u_batch, const int max_context);  
                auto ggml_backend_rpp_set_params_fn = (ggml_backend_rpp_set_params_t) ggml_backend_reg_get_proc_address(reg, "ggml_backend_set_params");
                if (ggml_backend_rpp_set_params_fn) {
                    // RPP_DOMAIN_TEXT = 1,
                    // RPP_DOMAIN_VISION = 2,
                    // RPP_DOMAIN_AUDIO = 3,
                    ggml_backend_rpp_set_params_fn(ctx_vision->backend, 2, -1, -1);
                }
            }
        ...
        if (loader.has_audio && !skip_audio) {
            ...
            if (reg) {
                ...
                if (ggml_backend_rpp_set_params_fn) {
                    ...
                    // RPP_DOMAIN_VISION = 2,
                    // RPP_DOMAIN_AUDIO = 3,
                    ggml_backend_rpp_set_params_fn(ctx_vision->backend, 3, -1, -1);
                }
            }
