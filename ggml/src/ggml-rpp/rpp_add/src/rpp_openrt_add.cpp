...
static bool ggml_rpp_create_engine_add_ubatch(ggml_backend_rpp_context & ctx,
                                              ggml_rpp_node *            rpp_base_node,
                                              ggml_tensor *              dst) {
    GGML_ASSERT(rpp_base_node);
    auto              rpp_node                     = static_cast<rpp_openrt_add *>(rpp_base_node);
    ...
    // create add operator
    auto opt_add = rpp_network->addElementWise(*inputs[0], *inputs[1], infer1::ElementWiseOperation::kSUM);
    if (!opt_add) {
...
static bool ggml_rpp_create_engine_add(ggml_backend_rpp_context & ctx,
                                       ggml_rpp_node *            rpp_base_node,
                                       ggml_tensor *              dst) {
    GGML_ASSERT(rpp_base_node);
    auto          rpp_node                     = static_cast<rpp_openrt_add *>(rpp_base_node);
    ...
    for (size_t i = 0; i < 2; i++) {
        ...
        inputs[i]->setType(ggml_rpp_get_io_type(ctx, dst->src[i], 0));
    }
    ...
    infer1::ITensor * ort_tensor = opt_add->getOutput(0);
    ort_tensor->setName(dst->name);
    ort_tensor->setType(ggml_rpp_get_io_type(ctx, dst, 1));
    rpp_network->markOutput(*ort_tensor);
...
static bool ggml_rpp_create_engine_dispatch(ggml_backend_rpp_context & ctx,
    ...
    if (!ret) {
        if (ctx.use_ubatch && ggml_rpp_add_seq_len(dst, rpp_node) > 1) {
            ret = ggml_rpp_create_engine_add_ubatch(ctx, rpp_node, dst);
        } else {
            ret = ggml_rpp_create_engine_add(ctx, rpp_node, dst);
        }
...
static bool ggml_rpp_set_io_bingdings_device(ggml_backend_rpp_context & ctx, ggml_rpp_node * rpp_base_node) {
    GGML_ASSERT(rpp_base_node);
    TRACE_SCOPE_GUARD(ctx.trace_id, "set_openrt_add_bingdings");
    auto rpp_node = static_cast<rpp_openrt_add *>(rpp_base_node);
    // clear buffers
...
static bool ggml_rpp_set_io_datas_device(ggml_backend_rpp_context & ctx, ggml_rpp_node * rpp_base_node) {
    GGML_ASSERT(rpp_base_node);
    TRACE_SCOPE_GUARD(ctx.trace_id, "set_openrt_add_datas");
    ...
    for (auto iter : rpp_node->binding_i_buffers) {
        ...
        if (!ggml_is_contiguous(i_tensor)) {
            ggml_rpp_pack_tensor_to_contiguous(ctx, i_tensor, iter.second);
        }
    }
}
...
bool ggml_rpp_op_openrt_add(ggml_backend_rpp_context & ctx, ggml_tensor * dst) {
    if (!dst) {
        GGML_LOG_ERROR("%s: ggml_tensor is nullptr %s (%s)\n", __func__, dst->name, ggml_op_name(dst->op));
        return false;
    }
    rpp_openrt_add * rpp_node = nullptr;
    ...
    if (iter == ctx.cur_rpp_graph->cur_rpp_nodes.end()) {
                    ...
                    if (node_iter.first->op == GGML_OP_ADD && ggml_rpp_dims_is_same(node_iter.first, dst)) {
                        ...
                        for (size_t i = 0; i < node_vec.size(); i++) {
                            ...
                            if (ggml_rpp_add_dims_is_same(dst, node_iter.first, cur_node)) {
                                auto new_node = std::make_unique<rpp_openrt_add>(dst, cur_node);
                                ctx.cur_rpp_graph->rpp_nodes[dst].emplace_back(std::move(new_node));
                                rpp_node = (rpp_openrt_add *) (ctx.cur_rpp_graph->rpp_nodes[dst].back().get());
                                // set io tensor
        ...
        if (!rpp_node) {
            TRACE_SCOPE_GUARD(ctx.trace_id, "create_openrt_add");
            auto new_node = std::make_unique<rpp_openrt_add>(dst);
            ctx.cur_rpp_graph->rpp_nodes[dst].emplace_back(std::move(new_node));
            rpp_node = (rpp_openrt_add *) (ctx.cur_rpp_graph->rpp_nodes[dst].back().get());
        ...
        rpp_node = (rpp_openrt_add *) (iter->second);
    ...
    try {
        TRACE_SCOPE_GUARD(ctx.trace_id, "execute_openrt_add");
