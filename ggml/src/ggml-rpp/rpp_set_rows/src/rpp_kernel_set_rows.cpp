...
#include "rpp_set_rows/src/rpp_kernel_build.h"

static int ggml_rpp_set_rows_seq_len(ggml_tensor * dst, ggml_rpp_node * node) {
    return dst->ne[node->seq_len_index];
}

static bool ggml_rpp_set_rows_properties_is_same(ggml_backend_rpp_context & ctx,
                                                 ggml_tensor *              dst,
    ...
    // so no need to use u_batch, only build on kernel for dst, just check the data ptr
...
static bool ggml_rpp_set_io_datas_device(ggml_backend_rpp_context & ctx, ggml_rpp_node * rpp_base_node) {
    ...
    auto      rpp_node                = static_cast<rpp_kernel_set_rows *>(rpp_base_node);
...
static bool ggml_rpp_create_kernel_set_rows(ggml_backend_rpp_context & ctx,
    ...
    rpp_set_rows_build(*(rpp_node->kernel_ctx.get()), cols, i_type_size, o_type_size, rpp_node->is_instantial);
    return true;
}
...
static bool ggml_rpp_create_kernel_dispatch(ggml_backend_rpp_context & ctx,
    ...
    if (ctx.use_ubatch && ggml_rpp_set_rows_seq_len(dst->src[0], rpp_node) > 1) {
        rpp_node->n_ubatch = ctx.n_ubatch;
    }
    ret = ggml_rpp_create_kernel_set_rows(ctx, rpp_node, dst);
    GGML_ASSERT(ret);
...
bool ggml_rpp_op_kernel_set_rows(ggml_backend_rpp_context & ctx, ggml_tensor * dst, int is_instantial, int is_launch) {
    if (!dst) {
        GGML_LOG_ERROR("%s: ggml_tensor is nullptr %s (%s)\n", __func__, dst->name, ggml_op_name(dst->op));
        return false;
    }
    rpp_kernel_set_rows * rpp_node = nullptr;
    ...
    if (iter == ctx.cur_rpp_graph->cur_rpp_nodes.end()) {
        ...
        if (iter_node != ctx.cur_rpp_graph->rpp_nodes.end()) {
            TRACE_SCOPE_GUARD(ctx.trace_id, "find_kernel_set_rows");
            ...
            for (size_t i = 0; i < node_vec.size(); i++) {
                ...
                if (cur_node->rpp_type == ggml_rpp_node::RPP_NODE_TYPE_KERNEL &&
                    ggml_rpp_set_rows_properties_is_same(ctx, dst, cur_node)) {
                    rpp_node = (rpp_kernel_set_rows *) cur_node;
                    break;
                }
            }
        }
        if (!rpp_node) {
            TRACE_SCOPE_GUARD(ctx.trace_id, "create_kernel_set_rows");
            auto new_node = std::make_unique<rpp_kernel_set_rows>(dst);
            ctx.cur_rpp_graph->rpp_nodes[dst].emplace_back(std::move(new_node));
            rpp_node                = (rpp_kernel_set_rows *) (ctx.cur_rpp_graph->rpp_nodes[dst].back().get());
        ...
        rpp_node = (rpp_kernel_set_rows *) (iter->second);
    }

    if (is_launch) {
        {
            TRACE_SCOPE_GUARD(ctx.trace_id, "set_io_datas_device_set_rows");
            ggml_rpp_set_io_datas_device(ctx, rpp_node);
        }
        // compute set_rows operator
        try {
            TRACE_SCOPE_GUARD(ctx.trace_id, "launch_kernel_set_rows");
