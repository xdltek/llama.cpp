...
static int ggml_rpp_l2_norm_seq_len(ggml_tensor * dst, ggml_rpp_node * node) {
...
static bool ggml_rpp_create_kernel_l2_norm(ggml_backend_rpp_context & ctx,
                                           ggml_rpp_node *            rpp_base_node,
                                           ggml_tensor *              dst) {
    GGML_ASSERT(rpp_base_node);
    auto rpp_node = static_cast<rpp_kernel_l2_norm *>(rpp_base_node);
    ...
    const int seq_len = ggml_rpp_l2_norm_seq_len(dst, rpp_node);
    int       rows    = 1;
    ...
    rpp_l2norm_build(*(rpp_node->kernel_ctx.get()), rows, cols, epsilon, i_type_size, o_type_size,
...
static bool ggml_rpp_create_kernel_dispatch(ggml_backend_rpp_context & ctx,
    ...
    if (ctx.use_ubatch && ggml_rpp_l2_norm_seq_len(dst, rpp_node) > 1) {
        rpp_node->n_ubatch = ctx.n_ubatch;
    }
    ret = ggml_rpp_create_kernel_l2_norm(ctx, rpp_node, dst);
    GGML_ASSERT(ret);
...
bool ggml_rpp_op_kernel_l2_norm(ggml_backend_rpp_context & ctx, ggml_tensor * dst, int is_instantial, int is_launch) {
    if (!dst) {
        GGML_LOG_ERROR("%s: ggml_tensor is nullptr %s (%s)\n", __func__, dst->name, ggml_op_name(dst->op));
        return false;
    }
    rpp_kernel_l2_norm * rpp_node = nullptr;
    ...
    if (iter == ctx.cur_rpp_graph->cur_rpp_nodes.end()) {
        ...
        if (iter_node != ctx.cur_rpp_graph->rpp_nodes.end()) {
            TRACE_SCOPE_GUARD(ctx.trace_id, "find_kernel_l2_norm");
            auto & node_vec = ctx.cur_rpp_graph->rpp_nodes[dst];
            for (size_t i = 0; i < node_vec.size(); i++) {
                auto cur_node = node_vec[i].get();
                if (cur_node->rpp_type == ggml_rpp_node::RPP_NODE_TYPE_KERNEL &&
                    ggml_rpp_l2_norm_properties_is_same(ctx, dst, cur_node)) {
                    rpp_node = (rpp_kernel_l2_norm *) cur_node;
                    break;
                }
            }
        }
        if (!rpp_node) {
            TRACE_SCOPE_GUARD(ctx.trace_id, "create_kernel_l2_norm");
            auto new_node = std::make_unique<rpp_kernel_l2_norm>(dst);
            ctx.cur_rpp_graph->rpp_nodes[dst].emplace_back(std::move(new_node));
            rpp_node                = (rpp_kernel_l2_norm *) (ctx.cur_rpp_graph->rpp_nodes[dst].back().get());
        ...
        rpp_node = (rpp_kernel_l2_norm *) (iter->second);
    }

    if (is_launch) {
        try {
            TRACE_SCOPE_GUARD(ctx.trace_id, "launch_kernel_l2_norm");
