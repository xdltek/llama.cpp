...
static bool ggml_rpp_create_engine_scale_ubatch(ggml_backend_rpp_context & ctx,
                                                ggml_rpp_node *            rpp_base_node,
                                                ggml_tensor *              dst) {
    GGML_ASSERT(rpp_base_node);
    auto rpp_node    = static_cast<rpp_openrt_scale *>(rpp_base_node);
    ...
    infer1::Weights shift{}, scale{}, power{};
    shift.type = infer1::DataType::kFLOAT;
    if (b) {
        shift.count = 1;
    } else {
        shift.count = 0;
    ...
    shift.values = &b;
    scale.type   = infer1::DataType::kFLOAT;
    if (s) {
        scale.count = 1;
    } else {
        scale.count = 0;
    ...
    scale.values = &s;
    power.type   = infer1::DataType::kFLOAT;
    if (p) {
        power.count = 1;
    } else {
        power.count = 0;
    ...
    const int         ndims = ggml_rpp_n_dims(dst->src);
    infer1::Dims      dims  = ggml_rpp_dims_mapping(src0, ndims, rpp_node);
    infer1::ITensor * input = ggml_rpp_create_input_tensor(src0, rpp_node, dims);
    if (!input) {
        GGML_LOG_ERROR("%s: creat input failed for scale, %s (%s)\n", __func__, dst->name, ggml_op_name(dst->op));
        return false;
    }

    // create add operator
    auto opt_add = rpp_network->addScale(*input, infer1::ScaleMode::kUNIFORM, shift, scale, power);
    if (!opt_add) {
        GGML_LOG_ERROR("%s: addElementWise add faied, %s (%s)\n", __func__, dst->name, ggml_op_name(dst->op));
        return false;
    }
    infer1::ITensor * ort_tensor = opt_add->getOutput(0);
...
static bool ggml_rpp_create_engine_scale(ggml_backend_rpp_context & ctx,
                                         ggml_rpp_node *            rpp_base_node,
                                         ggml_tensor *              dst) {
    GGML_ASSERT(rpp_base_node);
    auto rpp_node    = static_cast<rpp_openrt_scale *>(rpp_base_node);
    ...
    const int         ndims = ggml_rpp_n_dims(dst->src);
    infer1::ITensor * input = ggml_rpp_create_input_tensor(src0, rpp_node, ndims);
    if (!input) {
        GGML_LOG_ERROR("%s: creat input failed for scale, %s (%s)\n", __func__, dst->name, ggml_op_name(dst->op));
        return false;
    }
...
static bool ggml_rpp_create_engine_dispatch(ggml_backend_rpp_context & ctx,
    ...
    if (!ret) {
        if (ctx.use_ubatch && ggml_rpp_scale_seq_len(dst, rpp_node) > 1) {
            ret = ggml_rpp_create_engine_scale_ubatch(ctx, rpp_node, dst);
        } else {
            ret = ggml_rpp_create_engine_scale(ctx, rpp_node, dst);
        }
...
static bool ggml_rpp_set_io_bingdings_device(ggml_backend_rpp_context & ctx, ggml_rpp_node * rpp_base_node) {
    GGML_ASSERT(rpp_base_node);
    TRACE_SCOPE_GUARD(ctx.trace_id, "set_openrt_scale_bingdings");
    auto rpp_node = static_cast<rpp_openrt_scale *>(rpp_base_node);
    // clear buffers
...
static bool ggml_rpp_add_dims_is_same(ggml_tensor * dst, ggml_tensor * src, ggml_rpp_node * rpp_node) {
...
bool ggml_rpp_op_openrt_scale(ggml_backend_rpp_context & ctx, ggml_tensor * dst) {
    if (!dst) {
        GGML_LOG_ERROR("%s: ggml_tensor is nullptr %s (%s)\n", __func__, dst->name, ggml_op_name(dst->op));
        return false;
    }
    rpp_openrt_scale * rpp_node = nullptr;
    ...
    if (iter == ctx.cur_rpp_graph->cur_rpp_nodes.end()) {
        ...
        if (!rpp_node) {
            ...
            for (auto & graph_iter : ctx.gglm_graphs) {
                ...
                for (auto & node_iter : rpp_graph_tmp->rpp_nodes) {
                    ...
                    if (node_iter.first->op == GGML_OP_ADD && ggml_rpp_dims_is_same(node_iter.first, dst)) {
                        auto & node_vec = node_iter.second;
                        for (size_t i = 0; i < node_vec.size(); i++) {
                            auto cur_node = node_vec[i].get();
                            if (ggml_rpp_add_dims_is_same(dst, node_iter.first, cur_node)) {
                                auto new_node = std::make_unique<rpp_openrt_scale>(dst, cur_node);
                                ctx.cur_rpp_graph->rpp_nodes[dst].emplace_back(std::move(new_node));
                                rpp_node = (rpp_openrt_scale *) (ctx.cur_rpp_graph->rpp_nodes[dst].back().get());
                                // set io tensor
        ...
        if (!rpp_node) {
            TRACE_SCOPE_GUARD(ctx.trace_id, "create_openrt_scale");
            auto new_node = std::make_unique<rpp_openrt_scale>(dst);
            ctx.cur_rpp_graph->rpp_nodes[dst].emplace_back(std::move(new_node));
            rpp_node = (rpp_openrt_scale *) (ctx.cur_rpp_graph->rpp_nodes[dst].back().get());
        ...
        rpp_node = (rpp_openrt_scale *) (iter->second);
    ...
    try {
        TRACE_SCOPE_GUARD(ctx.trace_id, "execute_openrt_scale");
