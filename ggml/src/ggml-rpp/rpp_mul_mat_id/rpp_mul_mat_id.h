#ifndef RPP_MUL_MAT_ID
#define RPP_MUL_MAT_ID
...
struct rpp_kernel_mul_mat_id : public rpp_node_kernel {
    explicit rpp_kernel_mul_mat_id(ggml_tensor * tensor) : rpp_node_kernel(tensor) {}

    explicit rpp_kernel_mul_mat_id(ggml_tensor * tensor, ggml_rpp_node * rpp_node) :
        rpp_node_kernel(tensor, rpp_node) {}

    ~rpp_kernel_mul_mat_id() {
        if (sram_io) {
            delete sram_io;
            sram_io = nullptr;
        }
    ...
    bool rpp_dispatch_func(ggml_backend_rpp_context & ctx,
        ...
        return ggml_rpp_op_kernel_mul_mat_id(ctx, dst, is_instantial, is_launch);
    }
};

inline bool ggml_rpp_op_mul_mat_id(ggml_backend_rpp_context & ctx,
                                   struct ggml_tensor *       dst,
                                   int                        is_instantial = 1,
                                   int                        is_launch     = 1) {
    return ggml_rpp_op_kernel_mul_mat_id(ctx, dst, is_instantial, is_launch);
