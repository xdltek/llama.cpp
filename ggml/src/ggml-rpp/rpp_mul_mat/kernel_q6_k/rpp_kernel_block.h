...
//----------------------------------------------------------------------------------------------------
static void matmul_super_scale_blocks(uint32_t K,
...
static void matmul_q6k_dequant_blocks(uint32_t loop,
                                      uint32_t row,
                                      uint32_t column,
                                      dim3 &   threadsPerBlock,
                                      dim3 &   blocksPerGrid,
    ...
    if (block_y == 0) {
        throw std::runtime_error("matmul_q6k_dequant_blocks invalid weights_group");
    }
    ...
    if ((row % (uint32_t) sub_row_per_block) != 0) {
        throw std::runtime_error("matmul_q6k_dequant_blocks row is not divisible by block tile");
    }
