...
static void matmul_dequant_params(uint32_t                weight_in_addr,
    ...
    outStrideY       = elements_per_thread * block_x;
    outStrideZ       = elements_per_thread * block_x * block_y;
    //input block stride
    ...
    outBlockYSize     = elements_per_thread * block_x * block_y * block_z * in_type_of_bytes;
    //bfloat 16
    deQuantBlockSizeX = block_x * sizeof(short);
    deQuantBlockSizeY = column * block_z * sizeof(short);

    outBlockXSize -= (elements_per_thread - 1) * out_stride;
