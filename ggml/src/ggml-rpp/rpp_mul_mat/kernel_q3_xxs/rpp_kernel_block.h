...
static void q3xxs_super_scale_blocks(uint32_t K,
...
static void q3xxs_dequant_blocks(uint32_t loop,
                                 uint32_t row,
                                 uint32_t column,
                                 dim3 &   threadsPerBlock,
                                 dim3 &   blocksPerGrid,
    ...
    if ((row % (uint32_t) sub_row_per_block) != 0) {
        throw std::runtime_error("q3xxs_dequant_blocks row is not divisible by block tile");
    }
