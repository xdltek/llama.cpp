#pragma once
// rpp_matmul_q2s_vxm.cpp (v1 overlap: dmaStream + kernelStream + pingpong SRAM)

#include "ggml-rpp/rpp_kernel_ctx.h"
#include "ggml-rpp/rpp_kernel_utils.h"
#include "rpp_drv_api.h"
#include "rpp_mul_mat/kernel_q2_s_vxm/rpp_kernel_block.h"
#include "rpp_mul_mat/kernel_q2_s_vxm/rpp_kernel_param.h"
...
// Build graph once
// -----------------------------
static void rpp_matmul_q2s_vxm(rpp_kernel_context & ctx,
                               int                  M,
                               int                  K,
                               int                  N,
                               int                  weights_group,
    ...
    if ((M != 1) || (K / weights_group == 0) || (N % 32 != 0)) {
        throw std::runtime_error("Matmul Q2S Configure Paramter Invalid");
    }
    if (ctx.dev_in.size() < 6 || ctx.dev_out.empty()) {
        throw std::runtime_error("Matmul Q2S requires 6 inputs and 1 output");
    }
    ...
    const int sizeB_codebook_lsb = (Ktile * Ns / 8);
    const int sizeB_codebook_msb = (Ktile * Ns / 32);
    ...
    // Q2S scale tensor after super-scale is one bf16 per 16 weights.
    ...
    RPPdeviceptr sramB_codebook_lsb = sramC1 + round_up(sizeC);
    ...
    for (int i = 0; i < nr_of_tiles; i++) {
        for (int j = 0; j < nr_of_ns - 1; j++) {
            if (nr_of_ns == 1) {
                rtMemcpyAsync((void *) sramB_codebook_lsb, (const void *) devB_codebook_lsb, sizeB_codebook_lsb,
                              rtMemcpyDeviceToSram, ctx.kernelStream);
                rtMemcpyAsync((void *) sramB_codebook_msb, (const void *) devB_codebook_msb, sizeB_codebook_msb,
                              rtMemcpyDeviceToSram, ctx.kernelStream);
                ...
                devB_codebook_lsb += sizeB_codebook_lsb;
                devB_codebook_msb += sizeB_codebook_msb;
                devB_scales += sizeB_scales;
                devB_sign += sizeB_sign;
                devB_super_scale += sizeB_super_scale;
            } else {
                int       codebook_lsb_stride = (Ktile / 16 * N * (int) sizeof(short));
                int       codebook_msb_stride = (Ktile / 64 * N * (int) sizeof(short));
                int       scales_stride       = (Ktile / 64 * N * (int) sizeof(short));
                int       sign_stride         = (Ktile / 16 * N * (int) sizeof(short));
                int       super_scale_stride  = Ktile / weights_group * N * (int) sizeof(short);
                const int tile_ns_offset      = j * Ns * (int) sizeof(short);

                rtMemcpy2DAsync((void *) sramB_codebook_lsb, Ns * (int) sizeof(short),
                                (const void *) (devB_codebook_lsb + i * codebook_lsb_stride + tile_ns_offset),
                                N * (int) sizeof(short), Ns * (int) sizeof(short), Ktile / 16, rtMemcpyDeviceToSram,
                                ctx.kernelStream);

                rtMemcpy2DAsync((void *) sramB_codebook_msb, Ns * (int) sizeof(short),
                                (const void *) (devB_codebook_msb + i * codebook_msb_stride + tile_ns_offset),
            ...
            task.taskName           = "matrix_mul_vxM_f16_q2s_f16_asym_opt";
            ...
            matmul_weights_q2s_kernel_params(matmulBlocks, matmulThreads, sramA + i * stride_ina * groups_per_tile,
                                             sramB_codebook_lsb, sramB_codebook_msb, sramB_scale, sramB_sign,
                                             sramC + j * Ns * sizeof(short), sramB_grid_lut, input_acc_addr,
                                             input_acc_addr + hilo_offset, Ns, N * sizeof(short), weights_group,
                                             combine, task.params.kernelList);
    ...
    for (int i = 0; i < nr_of_tiles; i++) {
        if (nr_of_ns == 1) {
            ...
            rtMemcpy2DAsync((void *) sramB_codebook_lsb, NsTail * (int) sizeof(short),
                            (const void *) (devB_codebook_lsb + i * codebook_lsb_stride + tail_ns_offset),
                            N * (int) sizeof(short), NsTail * (int) sizeof(short), Ktile / 16, rtMemcpyDeviceToSram,
                            ctx.kernelStream);

            rtMemcpy2DAsync((void *) sramB_codebook_msb, NsTail * (int) sizeof(short),
                            (const void *) (devB_codebook_msb + i * codebook_msb_stride + tail_ns_offset),
                            N * (int) sizeof(short), NsTail * (int) sizeof(short), Ktile / 64, rtMemcpyDeviceToSram,
                            ctx.kernelStream);
        ...
        matmul_weights_q2s_kernel_params(matmulBlocks, matmulThreads, sramA + i * stride_ina * groups_per_tile,
                                         sramB_codebook_lsb, sramB_codebook_msb, sramB_scale, sramB_sign,
                                         sramC + (nr_of_ns - 1) * Ns * sizeof(short), sramB_grid_lut, input_acc_addr,
                                         input_acc_addr + hilo_offset, NsTail, N * sizeof(short), weights_group,
                                         combine, task.params.kernelList);
        launchWrapperAysnc(task.taskName, task.gridDim, task.blockDim, task.params.kernelList, ctx.rppBinMod,
                           ctx.kernelStream);
    }
...
static void rpp_matmul_q2s_vxm_build(rpp_kernel_context & ctx,
                                     int                  M,
                                     int                  K,
                                     int                  N,
                                     int                  weights_group,
                                     ...
                                     int                  use_pipeline = 0,
                                     int                  is_instantial = 1) {
    rpp_matmul_q2s_vxm(ctx, M, K, N, weights_group, in_bytes_per_element, out_bytes_per_element, is_instantial);
}
