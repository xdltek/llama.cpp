...
static void kerneldim_calc_matmul_q4k_step1(uint32_t   loop,
                                            uint32_t   column,
                                            uint32_t & block_x,
                                            uint32_t & block_y,
                                            uint32_t & block_z,
                                            ...
                                            uint32_t & grid_y,
                                            uint32_t & grid_z) {
    const uint32_t step1_loop_y = 8;
    block_y                     = step1_loop_y;
    if (column <= 512 && (column % 32) == 0) {
        block_x = column;
        grid_x  = 1;
    } else {
        block_x       = 32;
        uint32_t cntX = column / 32;
        while ((block_x < 512) && (cntX % 2 == 0) && ((block_x * 2 * step1_loop_y) <= 4096)) {
            block_x *= 2;
            cntX /= 2;
        }
        grid_x = cntX;
    ...
    assert((column % block_x) == 0);
    assert(block_x % 32 == 0);
}

static void kerneldim_calc_matmul_q4k_step2(uint32_t   loop,
                                            uint32_t   column,
                                            uint32_t & block_x,
                                            uint32_t & block_y,
                                            uint32_t & block_z,
                                            ...
                                            uint32_t & grid_y,
                                            uint32_t & grid_z) {
    kerneldim_calc_matmul_linear(loop, 1, column, block_x, block_y, block_z, grid_x, grid_y, grid_z);
}
}  // namespace kernel_q4_k_vxm
