...
static void q2xs_super_scale_blocks(uint32_t K,
...
static void q2xs_dequant_blocks(uint32_t loop,
                                uint32_t row,
                                uint32_t column,
                                dim3 &   threadsPerBlock,
                                dim3 &   blocksPerGrid,
                                int      weights_group) {
    (void) loop;
    uint32_t elements_per_thread = 16;
    ...
    if ((row % (uint32_t) sub_row_per_block) != 0) {
        throw std::runtime_error("q2xs_dequant_blocks row is not divisible by block tile");
    }
    ...
    if (grid_y == 0) {
        throw std::runtime_error("q2xs_dequant_blocks grid_y became zero");
    }
