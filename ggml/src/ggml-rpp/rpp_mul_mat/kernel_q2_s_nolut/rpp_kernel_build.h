...
#include "rpp_mul_mat/kernel_q2_s_nolut/rpp_kernel_block.h"
#include "rpp_mul_mat/kernel_q2_s_nolut/rpp_kernel_param.h"
...
static void rpp_matmul_q2s_nolut(rpp_kernel_context & ctx,
                                 int                  M,
                                 int                  K,
                                 int                  N,
                                 int                  weights_group,
    ...
    const int qscale_lut_bytes    = (int) q2s_nolut_lut_workspace::qscale_lut_bytes;
    const int mag_lut_bytes       = (int) q2s_nolut_lut_workspace::mag_lut_bytes;
    if (ctx.dev_in.size() < 5 || ctx.dev_out.empty()) {
        throw std::runtime_error("Q2S NoLUT requires 5 input buffers and 1 output buffer");
    }
    if (K % super_group != 0) {
        throw std::runtime_error("Q2S NoLUT requires K % 256 == 0");
    }
    if (weights_group != super_group) {
        throw std::runtime_error("Q2S NoLUT requires weights_group == 256");
    }
    const char * force_tn1_env = std::getenv("RPP_Q2S_NOLUT_FORCE_TN1");
    const bool   force_tn1     = (force_tn1_env != nullptr) && (std::atoi(force_tn1_env) != 0);
    if (force_tn1) {
        // std::cout << "[Q2S_NOLUT DBG] RPP_Q2S_NOLUT_FORCE_TN1=1, forcing tn1 for all tiles" << std::endl;
    }
    const char * force_ss_bx32_env = std::getenv("RPP_Q2S_NOLUT_SS_BX32");
    const bool   force_ss_bx32     = (force_ss_bx32_env != nullptr) && (std::atoi(force_ss_bx32_env) != 0);
    if (force_ss_bx32) {
        // std::cout << "[Q2S_NOLUT DBG] RPP_Q2S_NOLUT_SS_BX32=1, forcing q2s_nolut_super_scale block_x=32" << std::endl;
    }
    ...
    RPPdeviceptr dev_lut_workspace   = q2s_nolut_prepare_lut_workspace(ctx);

    RPPdeviceptr devB_qscale_lut = dev_lut_workspace;
    RPPdeviceptr devB_mag_lut    = devB_qscale_lut + qscale_lut_bytes;

    rppStreamBeginCapture(ctx.kernelStream, RPP_STREAM_CAPTURE_MODE_GLOBAL);
    rpp_module_load_once(ctx.rppBinMod, "rpp_kernel/matmul_q2s_nolut.o");

    const int sizeA0 = M * K * in_bytes_per_element;
    const int sizeA1 = M * K * (int) sizeof(rpp::bfloat16);

    // q2s_nolut packed inputs:
    // codebook_nolut: [K/8][N]  (uint16, one packed 2-bit-code word per 8 weights)
    ...
    for (int i = 0; i < get_tiles(N); i++) {
        ...
        if (i == 0) {
            // std::cout << "[Q2S_NOLUT DBG] tile ns=" << ns << " tn=" << tn << " kernel=" << matmul_kernel << std::endl;
        }
        ...
        params.clear();
        q2s_nolut_super_scale_blocks(K, super_group, q_group, ns, threadsPerBlock, blocksPerGrid);
        if (force_ss_bx32 && (ns % 32 == 0)) {
            ...
            if (i == 0) {
                // std::cout << "[Q2S_NOLUT DBG] super_scale override: block_x=" << threadsPerBlock.x
                //           << " grid_x=" << blocksPerGrid.x << std::endl;
            }
        }
        q2s_nolut_super_scale_params(sramB_scales, sramB_super_scale, sramB_qscale_lut, sramB_scale, K, ns, super_group,
                                     q_group, blocksPerGrid, threadsPerBlock, params);
        launchWrapperAysnc("matrix_mul_q2s_nolut_super_scale", blocksPerGrid, threadsPerBlock, params, ctx.rppBinMod,
                           ctx.kernelStream);

        // no-lut dequant: packed codebook/sign + super-scaled per-16 scale -> bf16 B matrix
        params.clear();
        q2s_nolut_dequant_blocks(1, K, ns, threadsPerBlock, blocksPerGrid, weights_group);
        q2s_nolut_dequant_params(blocksPerGrid, threadsPerBlock, sramB_codebook_nolut, sramB_sign, sramB_scale,
                                 sramB_out, sramB_mag_lut, ns, K, (int) sizeof(short), (int) sizeof(short), params);
        launchWrapperAysnc("matrix_mul_q2s_nolut_asym_opt", blocksPerGrid, threadsPerBlock, params, ctx.rppBinMod,
                           ctx.kernelStream);
...
static void rpp_matmul_q2s_nolut_sram(rpp_kernel_context & ctx,
                                      int                  M,
                                      int                  K,
                                      int                  N,
                                      int                  weights_group,
    ...
    const int super_group = 256;
    if (weights_group != super_group) {
        throw std::runtime_error("Q2S NoLUT SRAM requires weights_group == 256");
    }

    const char * force_tn1_env = std::getenv("RPP_Q2S_NOLUT_FORCE_TN1");
    const bool   force_tn1     = (force_tn1_env != nullptr) && (std::atoi(force_tn1_env) != 0);
    if (force_tn1) {
        // std::cout << "[Q2S_NOLUT DBG] RPP_Q2S_NOLUT_FORCE_TN1=1, forcing tn1 for SRAM-direct full-N launch"
        //           << std::endl;
    }
    const char * force_ss_bx32_env = std::getenv("RPP_Q2S_NOLUT_SS_BX32");
    const bool   force_ss_bx32     = (force_ss_bx32_env != nullptr) && (std::atoi(force_ss_bx32_env) != 0);
    ...
    const q2s_nolut_sram_io io =
        q2s_nolut_prepare_sram_io(ctx, M, K, N, weights_group, in_bytes_per_element, out_bytes_per_element);
    ...
    const int         tn            = force_tn1 ? 1 : get_tn(ns);
    const std::string matmul_kernel = force_tn1 ? "matmul_tn1_f16_f32_f16" : get_matmul_kernel(ns);
    // std::cout << "[Q2S_NOLUT DBG] SRAM-direct full-N ns=" << ns << " tn=" << tn << " kernel=" << matmul_kernel
    //             << std::endl;
    ...
    if (force_ss_bx32 && (ns % 32) == 0) {
        threadsPerBlock.x = 32;
        blocksPerGrid.x   = (uint32_t) (ns / 32);
        // std::cout << "[Q2S_NOLUT DBG] SRAM-direct super_scale override: block_x=" << threadsPerBlock.x
        //           << " grid_x=" << blocksPerGrid.x << std::endl;
    }
    q2s_nolut_super_scale_params((uint32_t) io.sramB_scales, (uint32_t) io.sramB_super_scale,
                                 (uint32_t) io.sramB_qscale_lut, (uint32_t) io.sramB_scale, K, ns, super_group, q_group,
    ...
    params.clear();
    q2s_nolut_dequant_blocks(1, K, ns, threadsPerBlock, blocksPerGrid, weights_group);
    q2s_nolut_dequant_params(blocksPerGrid, threadsPerBlock, (uint32_t) io.sramB_codebook_nolut,
...
static void rpp_matmul_q2s_nolut_build(rpp_kernel_context & ctx,
                                       int                  M,
                                       int                  K,
                                       int                  N,
                                       int                  weights_group,
                                       ...
                                       int                  is_instantial   = 1) {
    if (!use_sram_direct) {
        rpp_matmul_q2s_nolut(ctx, M, K, N, weights_group, in_bytes_per_element, out_bytes_per_element, is_instantial);
    } else {
        rpp_matmul_q2s_nolut_sram(ctx, M, K, N, weights_group, in_bytes_per_element, out_bytes_per_element,
                                  is_instantial);
    }
}
