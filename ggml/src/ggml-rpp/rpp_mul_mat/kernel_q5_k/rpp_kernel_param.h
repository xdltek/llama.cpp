...
static void q5k_super_scale_params(uint32_t                in_scale_lsb,
...
static void q5k_dequant_params(dim3 &                  blocksPerGrid,
                               dim3 &                  threadsPerBlock,
                               uint32_t                in_wq_lsb,
                               uint32_t                in_wq_msb,
                               uint32_t                scale,
    ...
    // in_wq_lsb     [z]          |  [y]  |  [unroll] | 4  |  [x]
    //               [grid.y]*[z] |  [y]                   |  [grid.x]*[x]
    //----------------------------------------------------------------------------------------------------
    // in_wq_msb     [K/256]      |  [8]  |  [32/16]  | 1  |  [16][N]
    // in_wq_msb     [z]          |  [y]  |  [unroll] | 1  |  [x]
    //               [grid.y]*[z] |  [y]                   |  [grid.x]*[x]
    //----------------------------------------------------------------------------------------------------
    int inStrideY0     = (32 / wqlsb_per_word) * column;
    int inStrideZ0     = 8 * inStrideY0;
    int inStrideY1     = (32 / wqmsb_per_word) * column;
    int inStrideZ1     = 8 * inStrideY1;

    int in_unroll_stride = column * sizeof(short);
    int inBlockXSize0    = block_x * sizeof(short);
    inBlockXSize0 -= (32 / wqlsb_per_word) * in_unroll_stride;
    int inBlockXSize1 = block_x * sizeof(short);
    inBlockXSize1 -= (32 / wqmsb_per_word) * in_unroll_stride;

    int inBlockYSize0 = block_y * block_z * (32 / wqlsb_per_word) * column * sizeof(short);
    inBlockYSize0 -= grid_x * block_x * sizeof(short);

    int inBlockYSize1 = block_y * block_z * (32 / wqmsb_per_word) * column * sizeof(short);
    inBlockYSize1 -= grid_x * block_x * sizeof(short);
    //----------------------------------------------------------------------------------------------------
    // out_addr   [N/32]   | [K/256]      | [8] | [32/16]    | [4x4]        | [32]
    // out_addr            | [z]          | [y] | [unroll0]  | [unroll1]    | [x]
    //            [grid.x] | [grid.y]*[z] | [y]                             | [x]
    //-----------------------------------------------------------------------------------------------------
    int outStrideY        = 32 * block_x;
