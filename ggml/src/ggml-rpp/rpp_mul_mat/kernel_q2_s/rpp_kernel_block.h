...
static void q2s_dequant_blocks(uint32_t loop,
                               uint32_t row,
                               uint32_t column,
                               dim3 &   threadsPerBlock,
                               dim3 &   blocksPerGrid,
    ...
    if ((row % (uint32_t) sub_row_per_block) != 0) {
        throw std::runtime_error("q2s_dequant_blocks row is not divisible by block tile");
    }
    ...
    if (grid_y == 0) {
        throw std::runtime_error("q2s_dequant_blocks grid_y became zero");
    }
