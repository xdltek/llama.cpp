...
static bool ggml_rpp_create_kernel_mul(ggml_backend_rpp_context & ctx,
                                       ggml_rpp_node *            rpp_base_node,
                                       ggml_tensor *              dst) {
    GGML_ASSERT(rpp_base_node);

    auto rpp_node = static_cast<rpp_kernel_mul *>(rpp_base_node);
    GGML_ASSERT(ggml_is_contiguous(dst->src[0]));
    GGML_ASSERT(ggml_is_contiguous(dst->src[1]));

    const int seq_len = ggml_rpp_mul_seq_len(dst, rpp_node);
    int       C       = 0;
    int       H       = 0;
    int       W       = 0;
    ...
    // build kernel
    rpp_elementwise_build(*(rpp_node->kernel_ctx.get()), RPP_ELEMWISE_MUL, C, H, W, axis, i_type_size_0, i_type_size_1,
...
static bool ggml_rpp_create_kernel_dispatch(ggml_backend_rpp_context & ctx,
    ...
    ret = ggml_rpp_create_kernel_mul(ctx, rpp_node, dst);
    GGML_ASSERT(ret);
...
static bool ggml_rpp_mul_properties_is_same(ggml_backend_rpp_context & ctx,
                                            ggml_tensor *              dst,
...
bool ggml_rpp_op_kernel_mul(ggml_backend_rpp_context & ctx, ggml_tensor * dst, int is_instantial, int is_launch) {
    if (!dst) {
        GGML_LOG_ERROR("%s: ggml_tensor is nullptr %s (%s)\n", __func__, dst->name, ggml_op_name(dst->op));
        return false;
    }
    rpp_kernel_mul * rpp_node = nullptr;
    ...
    if (iter == ctx.cur_rpp_graph->cur_rpp_nodes.end()) {
        ...
        if (iter_node != ctx.cur_rpp_graph->rpp_nodes.end()) {
            TRACE_SCOPE_GUARD(ctx.trace_id, "find_kernel_mul");
            ...
            for (size_t i = 0; i < node_vec.size(); i++) {
                ...
                if (cur_node->rpp_type == ggml_rpp_node::RPP_NODE_TYPE_KERNEL &&
                    ggml_rpp_mul_properties_is_same(ctx, dst, cur_node)) {
                    rpp_node = (rpp_kernel_mul *) cur_node;
                    break;
                }
            }
        }
        ...
        if (!rpp_node) {
            TRACE_SCOPE_GUARD(ctx.trace_id, "create_kernel_mul");
            auto new_node = std::make_unique<rpp_kernel_mul>(dst);
            ctx.cur_rpp_graph->rpp_nodes[dst].emplace_back(std::move(new_node));
            rpp_node                = (rpp_kernel_mul *) (ctx.cur_rpp_graph->rpp_nodes[dst].back().get());
        ...
        rpp_node = (rpp_kernel_mul *) (iter->second);
    }

    if (is_launch) {
        // compute add operator
        try {
            TRACE_SCOPE_GUARD(ctx.trace_id, "launch_kernel_mul");
